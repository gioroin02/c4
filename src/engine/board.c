#ifndef C4_ENGINE_BOARD_C
#define C4_ENGINE_BOARD_C
...
c4GameBoardHeight(C4GameBoard* self, pxiword column)
{
    ...
    for (pxiword i = 0; i < self->height; i += 1) {
        ...
        pxArrayGet(&self->grid, index, pxiword, &other);
...
pxb8
c4GameBoardGet(C4GameBoard* self, pxiword x, pxiword y, pxuword* value)
{
...
pxuword
c4GameBoardGetOr(C4GameBoard* self, pxiword x, pxiword y, pxuword value)
{
    pxuword result = 0;
